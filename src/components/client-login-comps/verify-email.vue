<script setup>
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await props.verifyEmail(form)
            console.log(response)
            if(props.error){
                errorMessage.value = props.error || 'Failed to verify email.';
                return
            }
...
<template>
    <form @submit="handleSubmit" class="form-container" aria-label="Client Login Form">
      <div class="form-header">
        <h2 v-if="!errorMessage && !isSuccess" class="h2-white">Email verification</h2>
        <div v-else>
          <ErrorMessage :msg="errorMessage" v-if="errorMessage"/>
          <h2 class="h2-white" v-if="isSuccess">URL sent to email</h2>
        </div>
      </div>
      <div>
        ...
        <button type="submit" class="btn-primary" aria-label="Submit login form">Verify</button>
      </div>
    </form>
</template>
